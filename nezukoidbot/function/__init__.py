...
from nezukoidbot.utils import load_module
...
from nezukoidbot import logging
...
async def apk_dl(app_name, path, event):
    ...
    for link in result[:1]:
        ...
        for link in result:
            ...
            with open(f"{path}/{name}@nezukoidbot.apk", "wb") as f:
                f.write(r.content)
    await event.edit("`Apk, Downloaded. Let me Upload It here.`")
    final_path = f"{path}/{name}@nezukoidbot.apk"
...
Music Downloaded And Uploaded By nezukoidbot
Get Your nezukoidbot From NezukoID"""
