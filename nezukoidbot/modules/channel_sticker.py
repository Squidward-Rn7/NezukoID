...
from nezukoidbot.modules.sql_helper.channel_sticker_sql import (
    add_new_data_in_db,
    is_data_indb,
    remove_datas,
)
from nezukoidbot.utils import admin_cmd

lulstark = [".", ",", "!", "'"]


@nezuko.on(admin_cmd(pattern="scs$"))
async def _m(event):
...
@nezuko.on(admin_cmd(pattern="rcs$"))
async def _m(event):
    await event.edit("`Processing..`")
    id_s = event.chat_id
    if is_data_indb(id_s):
...
@nezuko.on(admin_cmd(pattern="ccs$"))
async def _m(event):
    await event.edit("`Processing..`")
    id_s = event.chat_id
    if is_data_indb(id_s):
