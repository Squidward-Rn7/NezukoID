...
from nezukoidbot.function import fetch_feds
from nezukoidbot.modules.sql_helper.feds_sql import (
    add_fed,
    get_all_feds,
    is_fed_indb,
    rmfed,
...
@nezuko.on(nezuko_on_cmd(pattern="fadd ?(.*)"))
async def _(event):
    nolol = 0
    yeslol = 0
...
@nezuko.on(nezuko_on_cmd(pattern="frm ?(.*)"))
async def _(event):
    lol_s = event.pattern_match.group(1)
    await event.edit("`Processing..`")
    lol = get_all_feds()
...
@nezuko.on(nezuko_on_cmd(pattern="fban ?(.*)"))
async def _(event):
    lol_s = event.pattern_match.group(1)
    if lol_s == None:
...
@nezuko.on(nezuko_on_cmd(pattern="unfban ?(.*)"))
async def _(event):
    lol_s = event.pattern_match.group(1)
    if lol_s == None:
        await event.edit("`No User Found To Fban.`")
