...
from nezukoidbot import CMD_HELP, sclient
from nezukoidbot.utils import edit_or_reply, nezuko_on_cmd, sudo_cmd


@nezuko.on(nezuko_on_cmd("info ?(.*)"))
@nezuko.on(sudo_cmd("info ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@nezuko.on(nezuko_on_cmd("wru ?(.*)"))
@nezuko.on(sudo_cmd("wru ?(.*)", allow_sudo=True))
