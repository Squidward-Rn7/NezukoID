...
from nezukoidbot import ALIVE_NAME, CMD_HELP, CMD_LIST, lang
from nezukoidbot.function import _deezer_dl, _ytdl
from nezukoidbot.modules import inlinestats

PMPERMIT_PIC = os.environ.get("PMPERMIT_PIC", None)
if PMPERMIT_PIC is None:
    WARN_PIC = "https://telegra.ph/file/bcecc640c26dd9268941d.jpg"
else:
...
if lang == "si":
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query.startswith("NezukoID"):
            rev_text = query[::-1]
            ...
            result = builder.article(
                title="Stats",
                text=f"**Showing Stats For {DEFAULTUSER}'s NezukoIDBot** \nNote --> Only Owner Can Check This \n(C) [NezukoID](https://github.com/Squidward-Rn7/NezukoID)",
                buttons=[
                    [custom.Button.inline("Show Stats ?", data="terminator")],
                    [Button.url("Developed By", "https://github.com/Squidward-Rn7")],
                    [Button.url("Support Chat❤️", "https://t.me/NezukoIDBot")],
                ],
            )
            await event.answer([result])
    ...
    async def rip(event):
        ...
        link_s = yt_dl_data
        if event.query.user_id != bot.uid:
            text = f"Please Get Your Own NezukoID And Don't Waste My Resources"
            await event.answer(text, alert=True)
            return
        is_it = True
    ...
    async def sed(event):
        ...
        buttons = paginate_help(0, CMD_HELP, "helpme")
        sed = f"""NezukoID Modules Are Listed Here !\n
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("NezukoID"):
            ...
            result = builder.article(
                title="Stats",
                text=f"**Showing Stats For {DEFAULTUSER}'s NezukoID** \nNote --> Only Owner Can Check This \n(C) NezukoID",
                buttons=[
                    [custom.Button.inline("Show Stats ?", data="terminator")],
                    [
                        Button.url(
                            "Repo Here", "https://github.com/TeamEviral/NezukoID"
                        )
                    ],
                    [Button.url("Join Channel ❤️", "https://t.me/nezukoidchannel")],
                ],
            )
            await event.answer([result])
        elif event.query.user_id == bot.uid and query.startswith("**Hello"):
    ...
    async def on_plug_in_callback_query_handler(event):
        if not event.query.user_id == bot.uid:
            sedok = "Who The Fuck Are You? Get Your Own NezukoID ."
            await event.answer(sedok, cache_time=0, alert=True)
            return
        plugin_name = event.data_match.group(1).decode("UTF-8")
        if plugin_name in CMD_HELP:
        ...
        reply_pop_up_alert = help_string
        reply_pop_up_alert += "\n\n**(C) @nezukoidchannel** ".format(plugin_name)
        if len(reply_pop_up_alert) >= 4096:
            crackexy = "`Pasting Your Help Menu.`"
            await event.answer(crackexy, cache_time=0, alert=True)
            out_file = reply_pop_up_alert
    ...
    async def sed(event):
        ...
        # This Is Copy of Above Code. (C) @SpEcHiDe
        buttons = paginate_help(0, CMD_HELP, "helpme")
        sed = f"""NezukoID Userbot Modules Are Listed Here !\n
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy NezukoID To Get Your Own Assistant, Repo Link [Here](https://github.com/Squidward-Rn7/NezukoID)",
        )
