...
from nezukoidbot.modules.sql_helper.locks_sql import (
    get_locks,
    is_locked,
    update_lock,
)
...
@nezuko.on(nezuko_on_cmd("lock( (?P<target>\S+)|$)"))
@nezuko.on(sudo_cmd("lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@nezuko.on(nezuko_on_cmd("unlock ?(.*)"))
@nezuko.on(sudo_cmd("unlock ?(.*)", allow_sudo=True))
async def _(event):
...
@nezuko.on(nezuko_on_cmd("curenabledlocks"))
@nezuko.on(nezuko_on_cmd("curenabledlocks", allow_sudo=True))
async def _(event):
...
@nezuko.on(events.MessageEdited())  # pylint:disable=E0602
@nezuko.on(events.NewMessage())  # pylint:disable=E0602
...
@nezuko.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
