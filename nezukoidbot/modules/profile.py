...
@nezuko.on(nezuko_on_cmd(pattern="a2c(?: |$)(.*)"))
async def _(event):
...
@nezuko.on(nezuko_on_cmd(pattern="pbio (.*)"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@nezuko.on(nezuko_on_cmd(pattern="pname ((.|\n)*)"))  # pylint:disable=E0602,W0703
async def _(event):
    if event.fwd_from:
        return
...
@nezuko.on(nezuko_on_cmd(pattern="ppic"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
