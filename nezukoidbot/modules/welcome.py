...
from nezukoidbot.modules.sql_helper.welcome_sql import (
    add_welcome_setting,
...
@nezuko.on(nezuko_on_cmd(pattern="savewelcome"))
async def _(event):
    if event.fwd_from:
        return
...
@nezuko.on(nezuko_on_cmd(pattern="clearwelcome$"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
...
@nezuko.on(nezuko_on_cmd(pattern="listwelcome$"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
